...
export const createProduct = async (req, res) => {
  try {
    ...
    const product = new Product({
      ...req.body,
      discount: Number.isNaN(discountValue) ? 0 : Math.max(0, Math.min(100, discountValue)),
      images: imageUrls,
    });
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (updateData.discount !== undefined) {
      ...
      updateData.discount = Number.isNaN(parsedDiscount) ? 0 : Math.max(0, Math.min(100, parsedDiscount));
    }
